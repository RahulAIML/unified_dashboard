...
- [ ] **AI-generated dashboard branding: still hardcoded, not per-tenant.** `dashboard_config.py` sets `branding={"primary_color": "#DC2626"}` literally for every tenant; it never reads `lib/db-branding.ts`'s real per-tenant values. Unrelated to (and not fixed by) the personal-settings bug below.
...
- [~] **Broader cross-tenant sweep, 2026-07-31 (beyond Siigo/Apotex).** Generated live against 7 real tenants spanning all 5 connector kinds: Takeda (`rolplay_app_sql`, coach-only → 2 pages, "Overview"/"Master Coach" — a third distinct shape, proving page *titles* are module-driven too, not just page *count*), Besins (`coach_app_sql`/`rolplay_app_sql` ambiguity, see finding below), Gentera (`pharma_exceltis_rest`, no self-reported usecase IDs — correctly asked a human for them instead of guessing, see finding below), Heineken (`pharma_exceltis_rest`, cached IDs → real data, 0 errors), Sanfer (`pharma_sale_exercises`, richest tenant tested — certification + 975-row member table + 117-row admin table, 9/9 widgets real, 1 page since per-module scoping is `rolplay_app_sql`-only). All passed validation with real data; no tenant name/logic is hardcoded anywhere in the path exercised.
  - **Real finding, not yet fixed: primary-source selection is a fixed priority order, not confidence-scored.** Besins probed as BOTH `rolplay_app_sql` (client_id=11, 3 sessions, name-match) AND `coach_app_sql` (customer_id=16, **17 sessions**, domain-match) — `service_discovery.py::pick_primary()` picked `rolplay_app_sql` because match-*type* confidence (client-name match beats domain-LIKE match) outranks match *data volume*, a deliberate choice (confirmed correct for Takeda) but one that silently drops the richer secondary source's 17 real sessions from the dashboard entirely, with no warning surfaced anywhere the operator would see it. This is the concrete, live instance of exactly what the "generic architecture" ask means by "confidence-scored semantic mappings" — today it's a hardcoded 6-line priority table (`order = {"pharma_kpi": 0, ...}`), not an evidence-weighted decision, and it only ever picks ONE source rather than composing pages from both.
