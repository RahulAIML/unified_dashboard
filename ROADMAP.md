...
- [x] **Version/rollback read-path, 2026-07-31.** `dashboard_versions` has always gotten a row every publish; nothing ever read it back, so publishing was effectively irreversible. New `ai-service/app/dashboard_versions.py` + `GET/POST /ai/dashboard-versions/*` (proxied automatically through the existing generic `/api/ai/*` admin gate, no new Next.js route needed): list a slug's version history, roll back to restore an old version as current. Rollback is additive-only — it appends a NEW version rather than deleting anything, so a rollback is itself always undoable. Deliberately leaves `pharma_tenants`/domain routing untouched (a content rollback restores widgets/layout, not which data source serves the tenant). 11 new tests. **No UI wired to it yet** — usable today via the admin-gated API, not yet a button in `dashboard-builder`.
