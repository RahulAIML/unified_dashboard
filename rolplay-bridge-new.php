 ...
 * - NO raw SQL proxy endpoints
 * - Only allowlisted GET action endpoints
 * - Tenant resolution available only as action=resolve_customer_id (login-time)
 * - Tenant filtering enforced via JOIN-based customer_id scoping
 *
 * Authentication:
 * - Every request must include header: X-Bridge-Key = BRIDGE_SECRET
 *
 * Database:
 * - Primary DB (connected): rolplay_pro_analytics
...
switch ($action) {
  ...
  // Idempotent setup: create the contract view in rolplay_pro_analytics
  ...
  // Tenant resolution (login-time): email → customer_id via view
  ...
  // Overview KPIs: returns { current, prev } (raw avg_score; pass rate computed in app)
  case 'overview_kpis': {
  ...
  // Trends: returns raw series; app normalizes score
  case 'trends': {
    $customerId = parseIntParam('customer_id');
    $from = parseIsoToMysql('from');
    $to = parseIsoToMysql('to');
