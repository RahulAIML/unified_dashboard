 ...
 *
 * Toggle USE_REAL_DB to switch between real SQL and mock fallback.
 * If real DB fails, the provider returns mock data to keep UI alive.
 */
...
export const USE_REAL_DB = false
...
function getTrendsMock(filters: AnalyticsFilters): TrendsResult {
  ...
  const certification = getCertificationData(range)

  return {
    scoreTrend: simulator.scoreTrend as TrendPoint[],
    passFailTrend: certification.passFail as TrendPoint[],
...
function getUsecaseBreakdownMock(filters: AnalyticsFilters): UsecaseRow[] {
  const range = toRange(filters)
  const breakdown = getGlobalOverviewData(range).moduleBreakdown
...
function getEvaluationResultsMock(
  ...
  const range = toRange(filters)
  const rows = getCertificationData(range).resultsTable
...
// Real SQL implementations (unchanged queries)
export async function getOverviewKpis(
  filters: AnalyticsFilters
): Promise<OverviewKpis> {
  if (!USE_REAL_DB) return getOverviewKpisMock(filters)
  ...
  try {
    ...
    async function fetchPeriod(pFrom: Date, pTo: Date) {
      ...
      const passRate =
        totalRes > 0 ? Math.round((passed / totalRes) * 100 * 10) / 10 : null
    ...
    console.warn("[data-provider] getOverviewKpis failed, using mock", err)
...
export async function getTrends(
  ...
  if (!USE_REAL_DB) return getTrendsMock(filters)
  ...
  try {
    ...
    console.warn("[data-provider] getTrends failed, using mock", err)
...
export async function getEvaluationResults(
  ...
  if (!USE_REAL_DB) return getEvaluationResultsMock(filters, limit)
  ...
  try {
    ...
    console.warn("[data-provider] getEvaluationResults failed, using mock", err)
...
export async function getUsecaseBreakdown(
  ...
  if (!USE_REAL_DB) return getUsecaseBreakdownMock(filters)
  ...
  try {
    ...
    console.warn("[data-provider] getUsecaseBreakdown failed, using mock", err)
...
export async function getDrilldown(
  ...
  try {
    ...
    console.warn("[data-provider] getDrilldown failed, using mock", err)
    return null
  }
}

// Requested API-facing aliases
