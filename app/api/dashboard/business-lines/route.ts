...
// Pharma-sim only — no standard/Banco equivalent exists for a business-lines catalog.
export async function GET(request: NextRequest) {
  ...
  if (isDemoDataEnabled(ctx.email)) {
    ...
    return buildSuccess(range ? demoBusinessLines(range.from, range.to) : EMPTY, { source: 'demo' })
  }
  ...
  try {
    ...
    console.error('[/api/dashboard/business-lines]', err)
    return buildApiError('Failed to load business lines data')
  }
}
